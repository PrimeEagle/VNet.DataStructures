                                                                       ...
                                                                       where TValue : notnull, IComparable<TValue>
{
    public Path<TNode, TValue> FindShortestPath(IGraphSingleSourceShortestPathAlgorithmArgs<TNode, TEdge, TValue> args)
    {
        ...
        if (!predecessors.ContainsKey(args.EndNode)) return new Path<TNode, TValue>();
    ...
    public Dictionary<TNode, Path<TNode, TValue>> FindAllShortestPaths(IGraphSingleSourceShortestPathAlgorithmArgs<TNode, TEdge, TValue> args)
    {
        ...
        var shortestPaths = new Dictionary<TNode, Path<TNode, TValue>>();
        foreach (var node in args.Graph.Nodes)
        {
            if (!predecessors.ContainsKey(node))
            {
                shortestPaths[node] = new Path<TNode, TValue>();  // No path found.
                continue;
            }

            var path = new List<TNode> { node };
            while (!path.Last().Equals(args.StartNode)) path.Add(predecessors[path.Last()]);
            path.Reverse(); // Reverse the path to get it from start to end.
            shortestPaths[node] = new Path<TNode, TValue>(path);
        }
