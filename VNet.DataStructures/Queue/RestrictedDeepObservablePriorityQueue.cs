...
{
    public class RestrictedDeepObservablePriorityQueue<TItem, TPriority> : DeepObservableDoubleTypeCollectionBase<TItem, TPriority>
    {
        ...
        #region Constructors
        public RestrictedDeepObservablePriorityQueue()
        {
            _queue = new PriorityQueue<TItem, TPriority>();
        }

        public RestrictedDeepObservablePriorityQueue(IComparer<TPriority>? comparer)
        {
            _queue = new PriorityQueue<TItem, TPriority>(comparer);
        }

        public RestrictedDeepObservablePriorityQueue(IEnumerable<(TItem Element, TPriority Priority)> items)
        {
        ...
        public RestrictedDeepObservablePriorityQueue(int initialCapacity)
        {
            _queue = new PriorityQueue<TItem, TPriority>(initialCapacity);
        }

        public RestrictedDeepObservablePriorityQueue(int initialCapacity, IComparer<TPriority>? comparer)
        {
        ...
        public void EnqueueRange(IEnumerable<(TItem Element, TPriority Priority)> items)
        {
            ...
            if (_queue.Count == 0) _restrictedType = enumerable.ElementAt(0).GetType();
            if (enumerable.Any(i => i.GetType() != _restrictedType)) throw new ArgumentException("All types in a restricted queue must match.");

            var valueTuples = items as (TItem Element, TPriority Priority)[] ?? enumerable.ToArray();
